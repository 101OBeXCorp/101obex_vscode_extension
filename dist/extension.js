...
function activate(context) {
    const thisProvider = {
        resolveWebviewView: function (thisWebviewView, thisWebviewContext, thisToken) {
        ...
        sayHi: function (url) {
            ...
            if (url != null && url != '') {
                axios_1.default.get(url_config + AccesToken + pamameters_config, axiosConfig)
                    .then((response) => {
    ...
    try {
        context.subscriptions.push(vscode.commands.registerCommand('101obex-api-extension-api-specs.checkout-api-creator', async (e) => {
            ...
            if (API_FOLDER_ACTIVE === '') {
                vscode.window.showInformationMessage(`You cant checkout a commit outside his folder`, 'Open').then(async (selection) => {
                    if (selection == 'Open')
                        await vscode.commands.executeCommand('vscode.openFolder');
                });
                return;
            ...
            con2[0].APIs.forEach(api => {
                if (api.api_name == API_Name) {
                    api.history.forEach((version) => {
                        version.commits.forEach((element) => {
            ...
            commits, { placeHolder: 'Select Commit' }).then(async (pageType) => {
                ...
                if (pageType != undefined) {
                    ...
                    axios_1.default.get(`${cloud}/api_catalog/checkout?developer_token=` + AccesToken + `&obex_project_id=${SelectedProject}&api=${API_Name}&commit_id=${commit_id}`, axiosConfig)
                        .then(async (response) => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('101obex-api-extension-api-specs.viewPath', async (e) => {
        ...
        const iconPath = vscode.Uri.file(vscode.Uri.joinPath(context.extensionUri, 'images', 'AVAP-Specs-llaves.svg').fsPath);
        ...
        context.subscriptions.push(vscode.commands.registerCommand('101obex-api-extension-api-specs.commitPush-api-creator', async () => {
            ...
            if (valid) {
                ...
                if (commitDescription != undefined && commitDescription != "") {
                    ...
                    error = true;
                if (error) {
                    vscode.window.showErrorMessage('API Push aborted');
                }
                else {
                            ...
                            if (coont == files.length) {
                                ...
                                if (hasEntrypoint == true) {
                                    axios_1.default.post(`${cloud}/api_catalog/push_repo?developer_token=${AccesToken}&obex_project_id=${SelectedProject}&api=${APIPUSH}&message=${commitDescription}`, {
                                        ...
                                        vscode.commands.executeCommand('101obex-api-extension-api-publisher.refreshEntry-api-publisher');
                                        axios_1.default.get(url3 + AccesToken + `&obex_project_id=${SelectedProject}`, axiosConfig)
                                            .then((response) => {
                                            TokenData = response;
                ...
                vscode.window.showErrorMessage('You are not in an API Repository');
                // ERROR
            }
        }));
    }
        ...
        context.subscriptions.push(vscode.commands.registerCommand('101obex-api-extension-api-specs.addAPI-api-creator', async () => {
            ...
            if (error) {
                ...
                if (ApiName != undefined) {
                    if (API_NAMES_LIST.includes(ApiName))
                        message = `API ${ApiName} already exists.`;
                }
                if (endPoint != undefined && (API_ENDPOINT_LIST.includes(endPoint) || API_ENDPOINT_LIST.includes('/' + endPoint)))
                    message = `Endpoint ${endPoint} already used.`;
                vscode.window.showErrorMessage(`API Creation aborted ${message}`);
            }
        }));
    }
    catch { }
    ...
    if (ACCESS) {
        ...
        fs.readFile(configFile, 'utf8', (err, data) => {
            ...
            axios_1.default.get(url2 + dataObj.id_token + `&obex_project_id=${SelectedProject}`, axiosConfig)
                .then((response) => {
                TokenData = response;
                Services = response.data.data[0].services;
                var resultss = response.data.data[0].results;
                ...
                vscode.commands.registerCommand(`101obex-api-extension-api-specs.CloneAPI`, async (e) => {
                    ...
                    axios_1.default.get(/*url3*/ `${cloud}/api_catalog/clone?developer_token=` + dataObj.id_token + `&obex_project_id=${SelectedProject}&api=${APIClone}`, axiosConfig)
                        .then((response) => {
                        //console.log(response);
                        ...
                        try {
                            ...
                            vscode.window.showErrorMessage(`API ${APIClone} Already Cloned.`);
                        }
                    });
                    /*
                                    var arr:any;
...
*/
                axios_1.default.get(url3 + dataObj.id_token + `&obex_project_id=${SelectedProject}`, axiosConfig)
                    ...
                    vscode.commands.registerCommand(`101obex-api-extension-api-specs.CheckoutAPI`, async (e) => {
                        ...
                        if (!valid) {
                            vscode.window.showErrorMessage(`You are not in API ${API_Selected}, cant checkout`);
                        }
                        else {
                            vscode.window.showInformationMessage(`Checking Out ${e.tooltip.split('-')[1].substring(1, e.tooltip.length - 1)} Commit.`);
                            axios_1.default.get(`${cloud}/api_catalog/checkout?developer_token=` + dataObj.id_token + `&obex_project_id=${SelectedProject}&api=${API_Selected}&commit_id=${e.tooltip.split('-')[1].substring(1, e.tooltip.length - 1)}`, axiosConfig)
                                .then(async (response) => {
                                if (response.data.data.code != null) {
                                    let responseFinal = response.data.data.code.substring(1, response.data.data.code.length - 1);
                                    responseFinal = response.data.data.code.replace(/\\/g, '');
...
class TreeItem extends vscode.TreeItem {
    constructor(label, children, document, api_category, api_conection) {
        ...
        if (this.tooltip == "ADD_API_VIRTUAL") {
            this.iconPath = path.join(__filename, '..', '..', 'images', 'plus.png');
        }
        ...
        if (this.tooltip?.toString().split('|')[1] == 'connection') {
            ...
            if (this.children?.length === 1 &&
                ...
                if (this.tooltip?.toString().split('|').length > 1) {
                    if (this.tooltip?.toString().split('|')[1].toString() == 'local')
                        this.contextValue = 'UNPUBLISHED';
                }
            }
        }
...
function Connectors(context, response, thisProvider) {
    let porSel = getCurrentProject();
    SelectedProject = porSel.obex_project_id;
    SelectedProjectToken = porSel.selected_project;
    try {
    ...
    if (UPDATE_APIS) {
        ...
        axios_1.default.post(`${cloud}/api_catalog/init_repo?developer_token=${AccesToken}&obex_project_id=${SelectedProject}&api=${UPDATE_API_OBJ.apiname}&entrypoint=${UPDATE_API_OBJ.entrypoint}&endpoint=${UPDATE_API_OBJ.endpoint}`, {
...
.config-input::placeholder {
    ...
    <body onclick="">
        ...
        <div class="bento-box full-width" onclick="showMessage2()" id="product">
            ...
            ${details.tags.map((tag) => `<span class="label">${tag}</span>`).join('')}
        </div>
        ...
        <div class="bento-box" onclick="showMessage3()" id="consumes">
            ...
            <ul class="list">${details.consumes.map((type) => `<li class="config-value-list">${type}</li>`).join('')}</ul>
        </div>
...
The Produces section in Swagger specifies the media types or formats that the API can return in its response. It defines what type of data (such as application/json, application/xml, etc.) the server can provide to the client when the request is successful. This helps consumers understand the format of the response they can expect when interacting with the API.
            <ul class="list">${details.produces.map((type) => `<li class="config-value-list">${type}</li>`).join('')}</ul>
        </div>
        ...
        `<ul class="list config-value">${details.parameters.map((param) => `<li class="config-value-list"><strong>${param.name}</strong> (${param.in}) - ${param.description}</li>`).join('')}</ul>`
        : '<p>No parameters.</p>'}
        </div>
        ...
        if (!init) {
            try {
                axios_1.default.get(url_config + AccesToken + pamameters_config, axiosConfig)
                    ...
                    //console.log(api_parameters);
                    if (!init)
                        setTestData(`${cloud.replace(':3000', '')}/ws/low_code.py/` + url, api_parameters);
                    else
                        //ventanaNueva.webview.html = getWebviewContent(`${url}`,`${SelectedProjectToken}`,api_parameters);
                        setTestData("", [], init);
                });
