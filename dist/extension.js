        ...
        context.subscriptions.push(vscode.commands.registerCommand('101obex-api-extension-api-creation.checkout-api-creator', async (e) => {
            ...
            commits, { placeHolder: 'Select Commit' }).then(async (pageType) => {
                ...
                if (pageType != undefined) {
                    ...
                    axios_1.default.get('http://216.238.84.25:3000/api_catalog/checkout?developer_token=' + AccesToken + `&obex_project_id=${SelectedProject}&api=${API_Name}&commit_id=${commit_id}`, axiosConfig)
                        .then(async (response) => {
                    ...
                    files.forEach((file) => {
                        fs.readFile(file.fsPath, 'utf8', (err, data) => {
                            ...
                            if (coont == files.length) {
                                ...
                                axios_1.default.post(`http://216.238.84.25:3000/api_catalog/push_repo?developer_token=${AccesToken}&obex_project_id=${SelectedProject}&api=${APIPUSH}&message=${commitDescription}`, {
    ...
    try {
        context.subscriptions.push(vscode.commands.registerCommand('101obex-api-extension-api-creation.addAPI-api-creator', async () => {
            ...
            if (ApiName != undefined && ApiName != "") {
                ...
                error = true;
            if (error) {
                vscode.window.showErrorMessage('API Creation aborted');
            }
        }));
    }
    catch { }
        ...
        fs.readFile(configFile, 'utf8', (err, data) => {
            ...
            axios_1.default.get(url2 + dataObj.id_token + `&obex_project_id=${SelectedProject}`, axiosConfig)
                ...
                vscode.commands.registerCommand(`101obex-api-extension-api-creation.RemoveAPI`, async (e) => {
                    ...
                    axios_1.default.get(/*url3*/ 'http://216.238.84.25:3000/api_catalog/clone?developer_token=' + dataObj.id_token + `&obex_project_id=${SelectedProject}&api=${APIClone}`, axiosConfig)
                        .then((response) => {
                        //console.log(response);
                ...
                axios_1.default.get(url3 + dataObj.id_token + `&obex_project_id=${SelectedProject}`, axiosConfig)
                    ...
                    vscode.commands.registerCommand(`101obex-api-extension-api-creation.CheckoutAPI`, async (e) => {
                        ...
                        if (!valid) {
                            ...
                            axios_1.default.get('http://216.238.84.25:3000/api_catalog/checkout?developer_token=' + dataObj.id_token + `&obex_project_id=${SelectedProject}&api=${API_Selected}&commit_id=${e.tooltip.split('-')[1].substring(1, e.tooltip.length - 1)}`, axiosConfig)
                                .then(async (response) => {
                                if (response.data.data.code != null) {
                                    let responseFinal = response.data.data.code.substring(1, response.data.data.code.length - 1);
                                    responseFinal = response.data.data.code.replace(/\\/g, '');
...
function Connectors(context, response, thisProvider) {
    ...
    if (UPDATE_APIS) {
        ...
        axios_1.default.post(`http://216.238.84.25:3000/api_catalog/init_repo?developer_token=${AccesToken}&obex_project_id=${SelectedProject}&api=${UPDATE_API_OBJ.apiname}&entrypoint=${UPDATE_API_OBJ.entrypoint}&endpoint=${UPDATE_API_OBJ.endpoint}`, {
